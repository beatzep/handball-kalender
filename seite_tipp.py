...
(function () {
  ...
  bloecke.forEach(function (block) {
    ...
    //
    // Gespeichert ist der Tipp als Heim:Gast, nicht als Wir:Gegner - bei
    // einem Auswaertsspiel stuende die gegnerische Zahl dann zuerst und das
    // liest sich wie ein Tipp gegen die eigene Mannschaft. Hier wird darum
    // auf "wir zuerst" gedreht, mit letztesSpiel.heim aus dem umgebenden
    // Abschluss (siehe ladeTabelle).
    function letzterTippZeile(lt) {
      if (!lt || !lt.tipp || !letztesSpiel) return '';
      var wir = letztesSpiel.heim ? lt.tipp[0] : lt.tipp[1];
      var gegner = letztesSpiel.heim ? lt.tipp[1] : lt.tipp[0];
      return '<div class="letzter">zuletzt ' + wir + ':' + gegner
    ...
    function zeigeTabelle(zeilen) {
      ...
      if (letztesSpiel && letztesSpiel.ergebnis) {
        ...
        // ergebnis ist Heim:Gast gespeichert - auf Wir:Gegner gedreht wie im
        // eigenen Tipp darunter, sonst passen beide Zeilen nicht zusammen.
        var eigenesTor = letztesSpiel.heim ? letztesSpiel.ergebnis[0] : letztesSpiel.ergebnis[1];
        var gegnerTor = letztesSpiel.heim ? letztesSpiel.ergebnis[1] : letztesSpiel.ergebnis[0];
        kopf = '<p class="tippfuss">Letztes Spiel, ' + (letztesSpiel.heim ? 'gegen ' : 'bei ')
          + gegen + ': ' + eigenesTor + ':' + gegnerTor + '</p>';
      }
